...
with st.form("reg_form"):
    ...
    if st.form_submit_button("REGISTRAMI"):
        if email and pw and cellulare:
            try:
                supabase.auth.sign_up({"email": email, "password": pw, "options": {"data": {"nome": nome, "cognome": cognome, "cellulare": cellulare}}})
                st.success("✅ Registrato! Vai in Home per il login.")
